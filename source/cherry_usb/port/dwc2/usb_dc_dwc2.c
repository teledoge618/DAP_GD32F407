...
static void dwc2_set_txfifo(uint8_t fifo, uint16_t size) {
    ...
    ULOG_DEBUG("fifo%d size:%04x, offset:%04x\r\n", fifo, size, tx_offset);
}
...
static void dwc2_tx_fifo_empty_procecss(uint8_t ep_idx) {
    ...
    while (((USB_OTG_INEP(ep_idx)->DTXFSTS & USB_OTG_DTXFSTS_INEPTFSAV) >= len32b) && (g_dwc2_udc.in_ep[ep_idx].actual_xfer_len < g_dwc2_udc.in_ep[ep_idx].xfer_len) &&
...
/**
 * @brief  dwc2_get_outeps_intstatus: return the USB device OUT endpoints
 * interrupt status
 * @retval status
 */
...
/**
 * @brief  dwc2_get_ineps_intstatus: return the USB device IN endpoints
 * interrupt status
 * @retval status
 */
...
int usb_dc_init(uint8_t busid) {
    ...
    USB_OTG_GLB->GAHBCFG &= ~USB_OTG_GAHBCFG_GINT; /* 清全局中断使能 */
    ...
    USB_OTG_GLB->GCCFG |= USB_OTG_GCCFG_VBUSBSEN | USB_OTG_GCCFG_VBUSASEN | USB_OTG_GCCFG_NOVBUSSENS;
    ...
    USB_OTG_GLB->GINTMSK =
        USB_OTG_GINTMSK_USBRST | USB_OTG_GINTMSK_ENUMDNEM | USB_OTG_GINTMSK_OEPINT | USB_OTG_GINTMSK_IEPINT | USB_OTG_GINTMSK_IISOIXFRM | USB_OTG_GINTMSK_PXFRM_IISOOXFRM;

#ifdef CONFIG_USB_DWC2_DMA_ENABLE

    USB_OTG_DEV->DCFG &= ~USB_OTG_DCFG_DESCDMA;
    USB_OTG_GLB->GAHBCFG &= ~(USB_OTG_GAHBCFG_HBSTLEN);
    USB_OTG_GLB->GAHBCFG |= (USB_OTG_GAHBCFG_DMAEN | USB_OTG_GAHBCFG_HBSTLEN_3);
#else
    ...
    if (fifo_num > 0x3FF) {
        ULOG_ERROR("USB FIFO overflow\r\n");
    }

    ret = dwc2_flush_txfifo(0x10U);
    ret = dwc2_flush_rxfifo();

    USB_OTG_GLB->GAHBCFG |= USB_OTG_GAHBCFG_GINT; /* 全局中断使能 */
    USB_OTG_DEV->DCTL &= ~USB_OTG_DCTL_SDIS;
...
int usbd_ep_open(uint8_t busid, const struct usb_endpoint_descriptor *ep) {
    ...
    if (ep_idx > (CONFIG_USBDEV_EP_NUM - 1)) {
        ULOG_ERROR("Ep addr %02x overflow\r\n", ep->bEndpointAddress);
        return -1;
    }
    ...
    if (USB_EP_DIR_IS_OUT(ep->bEndpointAddress)) {
        ...
        if ((USB_OTG_OUTEP(ep_idx)->DOEPCTL & USB_OTG_DOEPCTL_USBAEP) == 0) {
            USB_OTG_OUTEP(ep_idx)->DOEPCTL |= (USB_GET_MAXPACKETSIZE(ep->wMaxPacketSize) & USB_OTG_DOEPCTL_MPSIZ) | ((uint32_t)USB_GET_ENDPOINT_TYPE(ep->bmAttributes) << 18) |
        ...
        if ((fifo_size * 4) < USB_GET_MAXPACKETSIZE(ep->wMaxPacketSize)) {
            ULOG_ERROR("Ep addr %02x fifo overflow\r\n", ep->bEndpointAddress);
            return -2;
        }
        ...
        if ((USB_OTG_INEP(ep_idx)->DIEPCTL & USB_OTG_DIEPCTL_USBAEP) == 0) {
            USB_OTG_INEP(ep_idx)->DIEPCTL |= (USB_GET_MAXPACKETSIZE(ep->wMaxPacketSize) & USB_OTG_DIEPCTL_MPSIZ) | ((uint32_t)USB_GET_ENDPOINT_TYPE(ep->bmAttributes) << 18) |
...
int usbd_ep_clear_stall(uint8_t busid, const uint8_t ep) {
    ...
    if (USB_EP_DIR_IS_OUT(ep)) {
        USB_OTG_OUTEP(ep_idx)->DOEPCTL &= ~USB_OTG_DOEPCTL_STALL;
        if ((g_dwc2_udc.out_ep[ep_idx].ep_type == USB_ENDPOINT_TYPE_INTERRUPT) || (g_dwc2_udc.out_ep[ep_idx].ep_type == USB_ENDPOINT_TYPE_BULK)) {
            USB_OTG_OUTEP(ep_idx)->DOEPCTL |= USB_OTG_DOEPCTL_SD0PID_SEVNFRM; /* DATA0 */
        }
    } else {
        USB_OTG_INEP(ep_idx)->DIEPCTL &= ~USB_OTG_DIEPCTL_STALL;
        if ((g_dwc2_udc.in_ep[ep_idx].ep_type == USB_ENDPOINT_TYPE_INTERRUPT) || (g_dwc2_udc.in_ep[ep_idx].ep_type == USB_ENDPOINT_TYPE_BULK)) {
...
void USBD_IRQHandler(uint8_t busid) {
    ...
    if ((USB_OTG_GLB->GINTSTS & 0x1U) == USB_OTG_MODE_DEVICE) {
        ...
        /*
        ULOG_ERROR("--------------------------------------\r\n");
        ULOG_ERROR("DIEPINT: 0x%0X\r\n", (USB_OTG_INEP(0)->DIEPINT));
        ULOG_ERROR("DTXFSTS: 0x%0X\r\n", (USB_OTG_INEP(0)->DTXFSTS));
        ULOG_ERROR("DIEPDMA: 0x%0X\r\n", (USB_OTG_INEP(0)->DIEPDMA));
        ULOG_ERROR("DIEPCTL: 0x%0X\r\n", (USB_OTG_INEP(0)->DIEPCTL));
        ULOG_ERROR("DIEPTSIZ: 0x%0X\r\n", (USB_OTG_INEP(0)->DIEPTSIZ));
        ULOG_ERROR("GRSTCTL: 0x%0X\r\n", USB_OTG_GLB->GRSTCTL);
        */

        /* OUT int, host to device */
        ...
        if (gint_status & USB_OTG_GINTSTS_IEPINT) {
            ...
            while (ep_intr != 0U) {
                if ((ep_intr & 0x1U) != 0U) {
                    ...
                    USB_OTG_INEP(ep_idx)->DIEPINT = DiepintReg; /* 清中断 */
        ...
        if (gint_status & USB_OTG_GINTSTS_PXFR_INCOMPISOOUT) {
            ...
            for (ep_idx = 1; ep_idx < CONFIG_USBDEV_EP_NUM; ep_idx++) {
                if (((1U << ep_idx) & ~daintmask) || (g_dwc2_udc.out_ep[ep_idx].ep_type != USB_ENDPOINT_TYPE_ISOCHRONOUS)) continue;
                if (!(USB_OTG_OUTEP(ep_idx)->DOEPCTL & USB_OTG_DOEPCTL_USBAEP)) continue;
        ...
        if (gint_status & USB_OTG_GINTSTS_IISOIXFR) {
            ...
            for (ep_idx = 1; ep_idx < CONFIG_USBDEV_EP_NUM; ep_idx++) {
                if ((((1U << ep_idx) & ~daintmask)) || (g_dwc2_udc.in_ep[ep_idx].ep_type != USB_ENDPOINT_TYPE_ISOCHRONOUS)) continue;

                if (!(USB_OTG_INEP(ep_idx)->DIEPCTL & USB_OTG_DIEPCTL_USBAEP)) continue;
