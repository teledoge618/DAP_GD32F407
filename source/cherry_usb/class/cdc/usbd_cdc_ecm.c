...
static USB_NOCACHE_RAM_SECTION USB_MEM_ALIGNX uint8_t
    g_cdc_ecm_rx_buffer[CONFIG_CDC_ECM_ETH_MAX_SEGSZE];
static USB_NOCACHE_RAM_SECTION USB_MEM_ALIGNX uint8_t
    g_cdc_ecm_tx_buffer[CONFIG_CDC_ECM_ETH_MAX_SEGSZE];
...
void usbd_cdc_ecm_send_notify(uint8_t notifycode, uint8_t value,
                              uint32_t *speed) {
    struct cdc_eth_notification *notify =
        (struct cdc_eth_notification *)g_cdc_ecm_notify_buf;
    ...
    if (bytes2send) {
        usbd_ep_start_write(0, cdc_ecm_ep_data[CDC_ECM_INT_EP_IDX].ep_addr,
                            g_cdc_ecm_notify_buf, bytes2send);
    }
}

static int cdc_ecm_class_interface_request_handler(
    uint8_t busid, struct usb_setup_packet *setup, uint8_t **data,
    uint32_t *len) {
    ULOG_DEBUG(
        "CDC ECM Class request: "
        "bRequest 0x%02x\r\n",
        setup->bRequest);
    ...
    switch (setup->bRequest) {
        case CDC_REQUEST_SET_ETHERNET_PACKET_FILTER:
            ...
            if (g_current_net_status == 0) {
                ...
                usbd_cdc_ecm_send_notify(CDC_ECM_NOTIFY_CODE_NETWORK_CONNECTION,
                                         CDC_ECM_NET_CONNECTED, NULL);
            }

            break;
        default:
            ULOG_WARN("Unhandled CDC ECM Class bRequest 0x%02x\r\n",
                     setup->bRequest);
            return -1;
    }
...
void cdc_ecm_notify_handler(uint8_t busid, uint8_t event, void *arg) {
    switch (event) {
        ...
        case USBD_EVENT_CONFIGURED:
            usbd_ep_start_read(
                0, cdc_ecm_ep_data[CDC_ECM_OUT_EP_IDX].ep_addr,
                &g_cdc_ecm_rx_buffer[g_cdc_ecm_rx_data_length],
                usbd_get_ep_mps(busid,
                                cdc_ecm_ep_data[CDC_ECM_OUT_EP_IDX].ep_addr));
            break;
...
void cdc_ecm_bulk_out(uint8_t busid, uint8_t ep, uint32_t nbytes) {
    ...
    if (nbytes < usbd_get_ep_mps(busid, ep)) {
        ...
        usbd_cdc_ecm_data_recv_done(g_cdc_ecm_rx_buffer,
                                    g_cdc_ecm_rx_data_length);
    } else {
        usbd_ep_start_read(0, ep,
                           &g_cdc_ecm_rx_buffer[g_cdc_ecm_rx_data_length],
...
void cdc_ecm_int_in(uint8_t busid, uint8_t ep, uint32_t nbytes) {
    if (g_current_net_status == 1) {
        ...
        usbd_cdc_ecm_send_notify(CDC_ECM_NOTIFY_CODE_NETWORK_CONNECTION,
                                 CDC_ECM_NET_CONNECTED, g_connect_speed_table);
    }
}
...
int usbd_cdc_ecm_start_write(uint8_t *buf, uint32_t len) {
    ...
    return usbd_ep_start_write(0, cdc_ecm_ep_data[CDC_ECM_IN_EP_IDX].ep_addr,
                               buf, g_cdc_ecm_tx_data_length);
}
...
void usbd_cdc_ecm_start_read_next(void) {
    ...
    g_cdc_ecm_rx_data_buffer = NULL;
    usbd_ep_start_read(
        0, cdc_ecm_ep_data[CDC_ECM_OUT_EP_IDX].ep_addr, g_cdc_ecm_rx_buffer,
        usbd_get_ep_mps(busid, cdc_ecm_ep_data[CDC_ECM_OUT_EP_IDX].ep_addr));
}
...
struct pbuf *usbd_cdc_ecm_eth_rx(void) {
    ...
    memcpy(p->payload, (uint8_t *)g_cdc_ecm_rx_buffer,
           g_cdc_ecm_rx_data_length);
...
int usbd_cdc_ecm_eth_tx(struct pbuf *p) {
    ...
    return usbd_cdc_ecm_start_write(g_cdc_ecm_tx_buffer,
                                    g_cdc_ecm_tx_data_length);
}
#endif

struct usbd_interface *usbd_cdc_ecm_init_intf(struct usbd_interface *intf,
