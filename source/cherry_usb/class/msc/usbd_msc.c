...
static int msc_storage_class_interface_request_handler(
    uint8_t busid, struct usb_setup_packet *setup, uint8_t **data,
    uint32_t *len) {
    ULOG_DEBUG(
        "MSC Class request: "
        "bRequest 0x%02x\r\n",
    ...
    switch (setup->bRequest) {
        ...
        default:
            ULOG_WARN("Unhandled MSC Class bRequest 0x%02x\r\n",
                     setup->bRequest);
            return -1;
    }
    ...
    switch (event) {
        case USBD_EVENT_INIT:
            ...
            if (g_usbd_msc[busid].usbd_msc_mq == NULL) {
                ULOG_ERROR(
                    "No memory to alloc for g_usbd_msc[busid].usbd_msc_mq\r\n");
            }
            g_usbd_msc[busid].usbd_msc_thread = usb_osal_thread_create(
                "usbd_msc", CONFIG_USBDEV_MSC_STACKSIZE, CONFIG_USBDEV_MSC_PRIO,
                usbdev_msc_thread, (void *)busid);
            if (g_usbd_msc[busid].usbd_msc_thread == NULL) {
                ULOG_ERROR(
                    "No memory to alloc for "
        ...
        case USBD_EVENT_CONFIGURED:
            ULOG_DEBUG("Start reading cbw\r\n");
            usbd_ep_start_read(
                busid, mass_ep_data[busid][MSD_OUT_EP_IDX].ep_addr,
                (uint8_t *)&g_usbd_msc[busid].cbw, USB_SIZEOF_MSC_CBW);
            break;
...
static void usbd_msc_bot_abort(uint8_t busid) {
    if ((g_usbd_msc[busid].cbw.bmFlags == 0) &&
        (g_usbd_msc[busid].cbw.dDataLength != 0)) {
    ...
    usbd_ep_start_read(busid, mass_ep_data[busid][0].ep_addr,
                       (uint8_t *)&g_usbd_msc[busid].cbw, USB_SIZEOF_MSC_CBW);
}
...
static void usbd_msc_send_csw(uint8_t busid, uint8_t CSW_Status) {
    ...
    ULOG_DEBUG("Send csw\r\n");
    usbd_ep_start_write(busid, mass_ep_data[busid][MSD_IN_EP_IDX].ep_addr,
                        (uint8_t *)&g_usbd_msc[busid].csw, sizeof(struct CSW));
}
...
static bool SCSI_inquiry(uint8_t busid, uint8_t **data, uint32_t *len) {
    ...
    uint8_t inquiry80[8] = {0x00, 0x80, 0x00,
                            0x08, 0x20, /* Put Product Serial number */
                            0x20, 0x20, 0x20};
    ...
    memcpy(&inquiry[8], CONFIG_USBDEV_MSC_MANUFACTURER_STRING,
           strlen(CONFIG_USBDEV_MSC_MANUFACTURER_STRING));
    memcpy(&inquiry[16], CONFIG_USBDEV_MSC_PRODUCT_STRING,
           strlen(CONFIG_USBDEV_MSC_PRODUCT_STRING));
    memcpy(&inquiry[32], CONFIG_USBDEV_MSC_VERSION_STRING,
           strlen(CONFIG_USBDEV_MSC_VERSION_STRING));
    ...
    if ((g_usbd_msc[busid].cbw.CB[1] & 0x01U) != 0U) { /* Evpd is set */
        if (g_usbd_msc[busid].cbw.CB[2] ==
            0U) { /* Request for Supported Vital Product Data Pages*/
            data_len = 0x06;
            memcpy(*data, (uint8_t *)inquiry00, data_len);
        } else if (g_usbd_msc[busid].cbw.CB[2] ==
...
static bool SCSI_startStopUnit(uint8_t busid, uint8_t **data, uint32_t *len) {
    ...
    } else if ((g_usbd_msc[busid].cbw.CB[4] & 0x3U) ==
               0x2U) /* START=0 and LOEJ Load Eject=1 */
    {
        // SCSI_MEDIUM_EJECTED;
        g_usbd_msc[busid].popup = true;
    } else if ((g_usbd_msc[busid].cbw.CB[4] & 0x3U) ==
               0x3U) /* START=1 and LOEJ Load Eject=1 */
    {
        // SCSI_MEDIUM_UNLOCKED;
    } else {
    }
...
static bool SCSI_preventAllowMediaRemoval(uint8_t busid, uint8_t **data,
                                          uint32_t *len) {
    if (g_usbd_msc[busid].cbw.dDataLength != 0U) {
        SCSI_SetSenseData(busid, SCSI_KCQIR_INVALIDCOMMAND);
        return false;
...
static bool SCSI_modeSense6(uint8_t busid, uint8_t **data, uint32_t *len) {
    ...
    uint8_t sense6[SCSIRESP_MODEPARAMETERHDR6_SIZEOF] = {0x03, 0x00, 0x00,
                                                         0x00};
...
static bool SCSI_modeSense10(uint8_t busid, uint8_t **data, uint32_t *len) {
    ...
    uint8_t sense10[27] = {0x00, 0x26, 0x00, 0x00, 0x00, 0x00, 0x00,
...
static bool SCSI_readFormatCapacity(uint8_t busid, uint8_t **data,
                                    uint32_t *len) {
    if (g_usbd_msc[busid].cbw.dDataLength == 0U) {
        SCSI_SetSenseData(busid, SCSI_KCQIR_INVALIDCOMMAND);
        return false;
    ...
    uint8_t format_capacity[SCSIRESP_READFORMATCAPACITIES_SIZEOF] = {
        ...
        (uint8_t)((g_usbd_msc[busid].scsi_blk_nbr[g_usbd_msc[busid].cbw.bLUN] >>
                   24) &
                  0xff),
        (uint8_t)((g_usbd_msc[busid].scsi_blk_nbr[g_usbd_msc[busid].cbw.bLUN] >>
                   16) &
        ...
        0x00,
        (uint8_t)((g_usbd_msc[busid]
                       .scsi_blk_size[g_usbd_msc[busid].cbw.bLUN] >>
                   8) &
                  0xff),
        (uint8_t)((g_usbd_msc[busid]
                       .scsi_blk_size[g_usbd_msc[busid].cbw.bLUN] >>
    ...
    memcpy(*data, (uint8_t *)format_capacity,
           SCSIRESP_READFORMATCAPACITIES_SIZEOF);
...
static bool SCSI_read10(uint8_t busid, uint8_t **data, uint32_t *len) {
    if (((g_usbd_msc[busid].cbw.bmFlags & 0x80U) != 0x80U) ||
        (g_usbd_msc[busid].cbw.dDataLength == 0U)) {
        SCSI_SetSenseData(busid, SCSI_KCQIR_INVALIDCOMMAND);
        return false;
    }

    g_usbd_msc[busid].start_sector =
        GET_BE32(&g_usbd_msc[busid]
                      .cbw.CB[2]); /* Logical Block Address of First Block */
    ULOG_DEBUG("lba: 0x%04x\r\n", g_usbd_msc[busid].start_sector);

    g_usbd_msc[busid].nsectors = GET_BE16(
        &g_usbd_msc[busid].cbw.CB[7]); /* Number of Blocks to transfer */
    ULOG_DEBUG("nsectors: 0x%02x\r\n", g_usbd_msc[busid].nsectors);
    ...
    if (g_usbd_msc[busid].cbw.dDataLength !=
        ...
        ULOG_ERROR("scsi_blk_len does not match with dDataLength\r\n");
        return false;
    }
...
static bool SCSI_read12(uint8_t busid, uint8_t **data, uint32_t *len) {
    ...
    g_usbd_msc[busid].nsectors = GET_BE32(
        &g_usbd_msc[busid].cbw.CB[6]); /* Number of Blocks to transfer */
    ULOG_DEBUG("nsectors: 0x%02x\r\n", g_usbd_msc[busid].nsectors);
...
static bool SCSI_write10(uint8_t busid, uint8_t **data, uint32_t *len) {
    uint32_t data_len = 0;
    if (((g_usbd_msc[busid].cbw.bmFlags & 0x80U) != 0x00U) ||
        (g_usbd_msc[busid].cbw.dDataLength == 0U)) {
        SCSI_SetSenseData(busid, SCSI_KCQIR_INVALIDCOMMAND);
        return false;
    }
    ...
    data_len = g_usbd_msc[busid].nsectors *
               g_usbd_msc[busid].scsi_blk_size[g_usbd_msc[busid].cbw.bLUN];
    if ((g_usbd_msc[busid].start_sector + g_usbd_msc[busid].nsectors) >
        g_usbd_msc[busid].scsi_blk_nbr[g_usbd_msc[busid].cbw.bLUN]) {
        ULOG_ERROR("LBA out of range\r\n");
    ...
    usbd_ep_start_read(busid, mass_ep_data[busid][MSD_OUT_EP_IDX].ep_addr,
                       g_usbd_msc[busid].block_buffer, data_len);
    return true;
}
...
static bool SCSI_processRead(uint8_t busid) {
    ...
    ULOG_DEBUG("read lba:%d\r\n", g_usbd_msc[busid].start_sector);

    transfer_len =
        MIN(g_usbd_msc[busid].nsectors *
                g_usbd_msc[busid].scsi_blk_size[g_usbd_msc[busid].cbw.bLUN],
            CONFIG_USBDEV_MSC_MAX_BUFSIZE);

    if (usbd_msc_sector_read(
            busid, g_usbd_msc[busid].cbw.bLUN, g_usbd_msc[busid].start_sector,
    ...
    usbd_ep_start_write(busid, mass_ep_data[busid][MSD_IN_EP_IDX].ep_addr,
                        g_usbd_msc[busid].block_buffer, transfer_len);
...
static bool SCSI_processWrite(uint8_t busid, uint32_t nbytes) {
    uint32_t data_len = 0;
    ULOG_DEBUG("write lba:%d\r\n", g_usbd_msc[busid].start_sector);

    if (usbd_msc_sector_write(busid, g_usbd_msc[busid].cbw.bLUN,
                              g_usbd_msc[busid].start_sector,
    ...
    g_usbd_msc[busid].start_sector +=
        (nbytes / g_usbd_msc[busid].scsi_blk_size[g_usbd_msc[busid].cbw.bLUN]);
    g_usbd_msc[busid].nsectors -=
        (nbytes / g_usbd_msc[busid].scsi_blk_size[g_usbd_msc[busid].cbw.bLUN]);
    ...
    if ((g_usbd_msc[busid].cbw.dSignature != MSC_CBW_Signature) ||
        ...
        switch (g_usbd_msc[busid].cbw.CB[0]) {
            ...
            case SCSI_CMD_PREVENTMEDIAREMOVAL:
                ret =
                    SCSI_preventAllowMediaRemoval(busid, &buf2send, &len2send);
                break;
            ...
            default:
                SCSI_SetSenseData(busid, SCSI_KCQIR_INVALIDCOMMAND);
                ULOG_WARN("unsupported cmd:0x%02x\r\n",
                         g_usbd_msc[busid].cbw.CB[0]);
                ret = false;
                break;
        }
    ...
    switch (g_usbd_msc[busid].stage) {
        case MSC_READ_CBW:
            if (SCSI_CBWDecode(busid, nbytes) == false) {
                ULOG_ERROR("Command:0x%02x decode err\r\n",
                          g_usbd_msc[busid].cbw.CB[0]);
            ...
            switch (g_usbd_msc[busid].cbw.CB[0]) {
                ...
                case SCSI_CMD_WRITE12:
                    ...
                    if (SCSI_processWrite(busid, nbytes) == false) {
                        usbd_msc_send_csw(
                            busid, CSW_STATUS_CMD_FAILED); /* send fail status
                                                              to host,and the
                                                              host will retry*/
                    }
#endif
...
static void usbdev_msc_thread(void *argument) {
    ...
    while (1) {
        ret = usb_osal_mq_recv(g_usbd_msc[busid].usbd_msc_mq,
                               (uintptr_t *)&event, USB_OSAL_WAITING_FOREVER);
        if (ret < 0) {
            continue;
        }
        ULOG_DEBUG("%d\r\n", event);
        if (event == MSC_DATA_OUT) {
            if (SCSI_processWrite(busid, g_usbd_msc[busid].nbytes) == false) {
                usbd_msc_send_csw(
                    busid,
                    CSW_STATUS_CMD_FAILED); /* send fail status to host,and the
                                               host will retry*/
            }
...
struct usbd_interface *usbd_msc_init_intf(uint8_t busid,
    ...
    for (uint8_t i = 0u; i <= g_usbd_msc[busid].max_lun; i++) {
        usbd_msc_get_cap(busid, i, &g_usbd_msc[busid].scsi_blk_nbr[i],
                         &g_usbd_msc[busid].scsi_blk_size[i]);

        if (g_usbd_msc[busid].scsi_blk_size[i] >
            CONFIG_USBDEV_MSC_MAX_BUFSIZE) {
            ULOG_ERROR("msc block buffer overflow\r\n");
            return NULL;
        }
    }
...
bool usbd_msc_set_popup(uint8_t busid) { return g_usbd_msc[busid].popup; }
