...
/**
 * usb_container_of - return the member address of ptr, if the type of ptr is
 * the struct type.
 */
 ...
 */
static inline void usb_slist_init(usb_slist_t *l) { l->next = NULL; }
...
static inline void usb_slist_insert(usb_slist_t *l, usb_slist_t *next,
                                    usb_slist_t *n) {
    if (!next) {
...
static inline usb_slist_t *usb_slist_head(usb_slist_t *l) { return l->next; }
...
static inline usb_slist_t *usb_slist_next(usb_slist_t *n) { return n->next; }

static inline int usb_slist_isempty(usb_slist_t *l) { return l->next == NULL; }
 ...
 * @brief initialize a slist object
 */
#define USB_SLIST_DEFINE(slist) usb_slist_t slist = {NULL}
 ...
 */
#define usb_slist_entry(node, type, member) usb_container_of(node, type, member)
 ...
 */
static inline void usb_dlist_init(usb_dlist_t *l) { l->next = l->prev = l; }
 ...
 */
#define USB_DLIST_OBJECT_INIT(object) {&(object), &(object)}
/**
 * @brief initialize a dlist object
 */
#define USB_DLIST_DEFINE(list) usb_dlist_t list = {&(list), &(list)}
 ...
 * @param member the name of list in structure
 */
#define usb_dlist_entry(node, type, member) usb_container_of(node, type, member)
...
/**
 * usb_dlist_for_each_safe - iterate over a list safe against removal of list
 * entry
 * @pos:    the dlist_t * to use as a loop cursor.
...
/**
 * usb_usb_dlist_for_each_entry_safe - iterate over list of given type safe
