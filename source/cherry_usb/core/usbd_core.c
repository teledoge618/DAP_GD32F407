...
static bool usbd_set_endpoint(uint8_t busid,
    ...
    ULOG_INFO("Open ep:0x%02x type:%u mps:%u\r\n", ep->bEndpointAddress,
...
static bool usbd_reset_endpoint(uint8_t busid,
                                const struct usb_endpoint_descriptor *ep) {
    ULOG_INFO("Close ep:0x%02x type:%u\r\n", ep->bEndpointAddress,
             USB_GET_ENDPOINT_TYPE(ep->bmAttributes));
...
static bool usbd_get_descriptor(uint8_t busid, uint16_t type_index,
    ...
    switch (type) {
        case USB_DESCRIPTOR_TYPE_DEVICE:
            g_usbd_core[busid].speed = usbd_get_port_speed(
                busid); /* before we get device descriptor, we have known steady
                           port speed */
        ...
        case USB_DESCRIPTOR_TYPE_STRING:
            if (index == USB_OSDESC_STRING_DESC_INDEX) {
                ULOG_INFO("read MS OS 2.0 descriptor string\r\n");
                ...
                desc = (uint8_t *)g_usbd_core[busid]
                           .descriptors->msosv1_descriptor->string;
                desc_len = g_usbd_core[busid]
                               .descriptors->msosv1_descriptor->string[0];
            } else {
                string =
                    g_usbd_core[busid].descriptors->string_descriptor_callback(
                ...
                if (total_size > CONFIG_USBDEV_REQUEST_BUFFER_LEN) {
                    ULOG_ERROR("string size overflow\r\n");
                    return false;
                }
        ...
        case USB_DESCRIPTOR_TYPE_DEVICE_QUALIFIER:
            ...
            return false;
#else
            desc = g_usbd_core[busid]
                       .descriptors->device_quality_descriptor_callback(
                           g_usbd_core[busid].speed);
            if (desc == NULL) {
                found = false;
                break;
        ...
        case USB_DESCRIPTOR_TYPE_OTHER_SPEED:
            desc =
                g_usbd_core[busid].descriptors->other_speed_descriptor_callback(
                    g_usbd_core[busid].speed);
            if (desc == NULL) {
                found = false;
                break;
        ...
        case USB_DESCRIPTOR_TYPE_BINARY_OBJECT_STORE:
            ULOG_INFO("read BOS descriptor string\r\n");
            ...
            desc = (uint8_t *)g_usbd_core[busid]
                       .descriptors->bos_descriptor->string;
            desc_len =
                g_usbd_core[busid].descriptors->bos_descriptor->string_len;
            break;
    ...
    if (found == false) {
        /* nothing found */
        ULOG_ERROR("descriptor <type:%x,index:%x> not found!\r\n", type, index);
    } else {
        *data = (uint8_t *)desc;
...
static bool usbd_get_descriptor(uint8_t busid, uint16_t type_index,
    ...
    if (found) {
        ...
        *data = p;
        ...
        /* nothing found */
        ULOG_ERROR("descriptor <type:0x%02x,index:0x%02x> not found!\r\n", type,
                  index);
    }
...
static bool usbd_set_interface(uint8_t busid, uint8_t iface,
    ...
    p = (uint8_t *)g_usbd_core[busid].descriptors;
#endif
    ULOG_DEBUG("iface %u alt_setting %u\r\n", iface, alt_setting);
    ...
    while (p[DESC_bLength] != 0U) {
        switch (p[DESC_bDescriptorType]) {
            ...
            case USB_DESCRIPTOR_TYPE_INTERFACE:
                ...
                ULOG_DEBUG("Current iface %u alt setting %u", cur_iface,
                          cur_alt_setting);
                break;
    ...
    switch (setup->bRequest) {
        ...
        case USB_REQUEST_SET_FEATURE:
            if (value == USB_FEATURE_REMOTE_WAKEUP) {
                if (setup->bRequest == USB_REQUEST_SET_FEATURE) {
                    g_usbd_core[busid].event_handler(
                        busid, USBD_EVENT_SET_REMOTE_WAKEUP);
                } else {
                    g_usbd_core[busid].event_handler(
                        busid, USBD_EVENT_CLR_REMOTE_WAKEUP);
                }
        ...
        case USB_REQUEST_SET_CONFIGURATION:
            ...
            if (!usbd_set_configuration(busid, value, 0)) {
                ...
                usbd_class_event_notify_handler(busid, USBD_EVENT_CONFIGURED,
                                                NULL);
    ...
    switch (setup->bRequest) {
        ...
        case USB_REQUEST_GET_DESCRIPTOR:
            if (type == 0x22) { /* HID_DESCRIPTOR_TYPE_HID_REPORT */
                ULOG_INFO("read hid report descriptor\r\n");
...
static int usbd_standard_request_handler(uint8_t busid,
    ...
    switch (setup->bmRequestType & USB_REQUEST_RECIPIENT_MASK) {
        ...
        case USB_REQUEST_RECIPIENT_INTERFACE:
            if (usbd_std_interface_req_handler(busid, setup, data, len) ==
                false) {
                rc = -1;
            }
        ...
        case USB_REQUEST_RECIPIENT_ENDPOINT:
            if (usbd_std_endpoint_req_handler(busid, setup, data, len) ==
                false) {
                rc = -1;
            }
...
static int usbd_vendor_request_handler(uint8_t busid,
    ...
    if (g_usbd_core[busid].descriptors->msosv1_descriptor) {
        if (setup->bRequest ==
            ...
            switch (setup->wIndex) {
                case 0x04:
                    ULOG_INFO("get Compat ID\r\n");
                    desclen =
                        g_usbd_core[busid]
                            .descriptors->msosv1_descriptor->compat_id[0] +
                        (g_usbd_core[busid]
                             .descriptors->msosv1_descriptor->compat_id[1]
                         << 8) +
                        (g_usbd_core[busid]
                             .descriptors->msosv1_descriptor->compat_id[2]
                         << 16) +
                        (g_usbd_core[busid]
                             .descriptors->msosv1_descriptor->compat_id[3]
                         << 24);
                    ...
                    *len = desclen;
                    return 0;
                case 0x05:
                    ULOG_INFO("get Compat id properties\r\n");
                    ...
                    *len = desclen;
                    return 0;
                default:
                    ULOG_ERROR("unknown vendor code\r\n");
                    return -1;
            }
        }
        ...
        if (setup->bRequest ==
            ...
            switch (setup->wIndex) {
                case WINUSB_REQUEST_GET_DESCRIPTOR_SET:
                    ULOG_INFO("GET MS OS 2.0 Descriptor\r\n");

                    desclen =
                        g_usbd_core[busid]
                            .descriptors->msosv2_descriptor->compat_id_len;
                    *data = (uint8_t *)g_usbd_core[busid]
                                .descriptors->msosv2_descriptor->compat_id;
                    // memcpy(*data,
                    // g_usbd_core[busid].descriptors->msosv2_descriptor->compat_id,
                    // desclen);
                    *len = g_usbd_core[busid]
                               .descriptors->msosv2_descriptor->compat_id_len;
                    return 0;
                default:
                    ULOG_ERROR("unknown vendor code\r\n");
        ...
        if (setup->bRequest ==
            g_usbd_core[busid]
                .descriptors->webusb_url_descriptor->vendor_code) {
            switch (setup->wIndex) {
                case WINUSB_REQUEST_GET_DESCRIPTOR_SET:
                    ULOG_INFO("GET Webusb url Descriptor\r\n");

                    desclen =
                        g_usbd_core[busid]
                            .descriptors->webusb_url_descriptor->string_len;
                    *data = (uint8_t *)g_usbd_core[busid]
        ...
        if (setup->bRequest == g_usbd_core[busid].msosv1_desc->vendor_code) {
            switch (setup->wIndex) {
                case 0x04:
                    ULOG_INFO("get Compat ID\r\n");
                    *data =
                        (uint8_t *)g_usbd_core[busid].msosv1_desc->compat_id;
                    desclen =
                        g_usbd_core[busid].msosv1_desc->compat_id[0] +
                    ...
                    // memcpy(*data, g_usbd_core[busid].msosv1_desc->compat_id,
                    // desclen);
                    *len = desclen;
                    return 0;
                case 0x05:
                    ...
                    *data = (uint8_t *)g_usbd_core[busid]
                    ...
                    desclen =
                        g_usbd_core[busid]
                            .msosv1_desc->comp_id_property[setup->wValue][0] +
                        (g_usbd_core[busid]
                             .msosv1_desc->comp_id_property[setup->wValue][1]
                         << 8) +
                        (g_usbd_core[busid]
                             .msosv1_desc->comp_id_property[setup->wValue][2]
                         << 16) +
                        (g_usbd_core[busid]
                             .msosv1_desc->comp_id_property[setup->wValue][3]
                         << 24);
                    // memcpy(*data,
        ...
        if (setup->bRequest == g_usbd_core[busid].msosv2_desc->vendor_code) {
            switch (setup->wIndex) {
                case WINUSB_REQUEST_GET_DESCRIPTOR_SET:
                    ULOG_INFO("GET MS OS 2.0 Descriptor\r\n");
                    *data =
                        (uint8_t *)g_usbd_core[busid].msosv2_desc->compat_id;
    ...
    switch (setup->bmRequestType & USB_REQUEST_TYPE_MASK) {
        case USB_REQUEST_STANDARD:
            if (usbd_standard_request_handler(busid, setup, data, len) < 0) {
                /* Ignore error log for getting Device Qualifier Descriptor
                 * request */
                if ((setup->bRequest == 0x06) && (setup->wValue == 0x0600)) {
                    // ULOG_DEBUG("Ignore DQD in fs\r\n");
                    return false;
                }
                ULOG_ERROR("standard request error\r\n");
        ...
        case USB_REQUEST_CLASS:
            if (usbd_class_request_handler(busid, setup, data, len) < 0) {
                ULOG_ERROR("class request error\r\n");
                usbd_print_setup(setup);
                return false;
            }
            break;
        case USB_REQUEST_VENDOR:
            if (usbd_vendor_request_handler(busid, setup, data, len) < 0) {
                ULOG_ERROR("vendor request error\r\n");
                usbd_print_setup(setup);
                return false;
            }
            break;
...
void usbd_event_ep0_setup_complete_handler(uint8_t busid, uint8_t *psetup) {
    ...
    if (setup->wLength > CONFIG_USBDEV_REQUEST_BUFFER_LEN) {
        if ((setup->bmRequestType & USB_REQUEST_DIR_MASK) ==
            USB_REQUEST_DIR_OUT) {
            ULOG_ERROR("Request buffer too small\r\n");
    ...
    if (setup->wLength && ((setup->bmRequestType & USB_REQUEST_DIR_MASK) ==
                           USB_REQUEST_DIR_OUT)) {
        ULOG_DEBUG("Start reading %d bytes from ep0\r\n", setup->wLength);
    ...
    /* use *data = xxx; g_usbd_core[busid].ep0_data_buf records real data
     * address, we should copy data into ep0 buffer. Why we should copy once?
     * because some chips are not access to flash with dma if real data address
     * is in flash address(such as ch32).
     */
    if (buf != g_usbd_core[busid].ep0_data_buf) {
#ifdef CONFIG_USBDEV_EP0_INDATA_NO_COPY
        g_usbd_core[busid].ep0_data_buf = buf;
#else
        memcpy(g_usbd_core[busid].ep0_data_buf, buf,
               g_usbd_core[busid].ep0_data_buf_residue);
#endif
    } else {
        /* use memcpy(*data, xxx, len); has copied into ep0 buffer, we do
         * nothing */
    }
    ...
    if ((setup->wLength > g_usbd_core[busid].ep0_data_buf_len) &&
        ...
        ULOG_DEBUG("EP0 Set zlp\r\n");
    }
}

void usbd_event_ep0_in_complete_handler(uint8_t busid, uint8_t ep,
                                        uint32_t nbytes) {
    struct usb_setup_packet *setup = &g_usbd_core[busid].setup;
    ...
    ULOG_DEBUG("EP0 send %d bytes, %d remained\r\n", nbytes,
              g_usbd_core[busid].ep0_data_buf_residue);
    ...
    if (g_usbd_core[busid].ep0_data_buf_residue != 0) {
        ...
        if (g_usbd_core[busid].zlp_flag == true) {
             ...
             */
            if (setup->wLength &&
                ((setup->bmRequestType & USB_REQUEST_DIR_MASK) ==
                 USB_REQUEST_DIR_IN)) {
...
void usbd_event_ep0_out_complete_handler(uint8_t busid, uint8_t ep,
                                         uint32_t nbytes) {
    struct usb_setup_packet *setup = &g_usbd_core[busid].setup;
    ...
    if (nbytes > 0) {
        ...
        ULOG_DEBUG("EP0 recv %d bytes, %d remained\r\n", nbytes,
                  g_usbd_core[busid].ep0_data_buf_residue);

        if (g_usbd_core[busid].ep0_data_buf_residue == 0) {
            /* Received all, send data to handler */
            g_usbd_core[busid].ep0_data_buf = g_usbd_core[busid].req_data;
            if (!usbd_setup_request_handler(
                    busid, setup, &g_usbd_core[busid].ep0_data_buf,
                    &g_usbd_core[busid].ep0_data_buf_len)) {
                usbd_ep_set_stall(busid, USB_CONTROL_IN_EP0);
                return;
            }
        ...
        ULOG_DEBUG("EP0 recv out status\r\n");
    }
}
